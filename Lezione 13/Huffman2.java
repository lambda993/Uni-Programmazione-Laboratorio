...
public class Huffman2 {
  ...
  private static void fillTable( Node n, String code, String[] codes ) {
    ...
    if ( n.foglia() ) {
      codes[ n.getCarattere() ] = code;                        // nodo foglia: code = codice del carattere
    } else {
      fillTable( n.getSinistro(),  code+"0", codes );              // figlio sinistro: bit 0 nel codice
      fillTable( n.getDestro(), code+"1", codes );              // figlio destro:   bit 1 nel codice
    }
  }
  ...
  public static String flattenTree( Node n ) {
    ...
    if ( n.foglia() ) {                                     // foglia: codifica del carattere
      ...
      return ( "@"                                          // @
             + flattenTree( n.getSinistro() )                      // codifica lineare del sottoalbero sinistro
             + flattenTree( n.getDestro() )                     // codifica lineare del sottoalbero destro
               );
    }
  }
  ...
  public static void compress( String src, String dst ) {
    ...
    int count = root.getPeso();                              // numero complessivo di caratteri
  ...
  private static char decodeNextChar( Node root, InputTextFile in ) {
    ...
    do {
      if ( in.readBit() == 0 ) {
        n = n.getSinistro();                                       // bit 0: figlio sinistro
      } else {
        n = n.getDestro();                                      // bit 1: figlio destro
      }
    } while ( !n.foglia() );                                // fino al raggiungimento di una foglia
    
    return n.getCarattere();                                   // carattere individuato
  }
...
/*
Huffman2.compress("Sample.txt", "CompressedSample.txt");
Huffman2.decompress("CompressedSample.txt", "DeCompressedSample.txt");
*/
